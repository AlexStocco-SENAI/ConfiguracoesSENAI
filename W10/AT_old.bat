@ECHO OFF
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/00-INICIO.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/02-Identificacao.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/03-Plano_de_Fundo.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/04-Conexao.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/05-Programas.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/06-Bloqueios.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/07-LanSchool.bat > nul 2>&1
@curl -LJO https://alexstocco-senai.github.io/ConfiguracoesSENAI/Estruturar/08-Particao.bat > nul 2>&1

